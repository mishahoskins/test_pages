...
wastewaterfile <- paste0(
    "./wastewater/CASE_address_wastewater_",
    Sys.Date(),
    ".xlsx"
)
...
linelist <- read.csv(
    "T:\\VPDs\\Measles\\Cases, Clusters, and Outbreaks\\Dec 2025 - Jan 2026 Outbreak\\Data\\Linelists\\All_Models_Identified_Cases_and_Contacts_Line_List_by_Event_Create_Date_20260206100621.xls.csv"
) # Update file names here
# reading in vaccine info
vaccine <- read.csv(
    "T:\\VPDs\\Measles\\Cases, Clusters, and Outbreaks\\Dec 2025 - Jan 2026 Outbreak\\Data\\Linelists\\Case_Information_Extract_Excel_CSV_20260206102704.xls.csv"
) # Update file names here
...
linelist_vaccine <- linelist |>
    mutate(
        ...
        week_symptom = ceiling_date(mdy(earliest_id_date), unit = "week") -
            days(1),
...
epi_curve_1 <- table_view |>
    mutate(
        week_symptom = floor_date(week_symptom, "week", week_start = 1) +
            days(7)
    ) |>
...
epi_curve_cum <- table_view |>
    ...
    mutate(
        week_symptom = floor_date(week_symptom, "week") + days(6),
...
epi_prep <- linelist_vaccine2 |>
    mutate(
        ...
        vax_stat = case_when(
            VACCINE_NUMBER_DOSES %in%
                c('0') ~ 'No Evidence of Immunty/ Unknown',
            VACCINE_NUMBER_DOSES %in% c('1') ~ '1 dose of MMR',
...
##vaccination status with legend underneath
epi_curve_vax <- epi_prep |>
    ggplot(aes(x = county, y = Cases, fill = vax_stat)) +

    geom_col(width = 0.9) +

    scale_fill_manual(values = c('#c7efcf', '#7d938a', '#8fd3c7')) +
    ...
    labs(
        title = "Immunity Status of Cases",
        x = "County of Residence",
        y = "Case count",
        fill = "Immunity Status"
    ) +
...
# Moved to end of code so we don't accidently overwrite files before they're ready
# send to wastewaterfolder
# write.xlsx(x = case_address, file = wastewaterfile)

# Make a list for a single export for linelist
#vaccination_stat_combine <- list(
#   "linelist" = linelist_vaccine2,
#  "weekly count" = table_view
#)
